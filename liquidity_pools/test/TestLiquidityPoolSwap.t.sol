...
contract LiquidityPoolTest is Test, ERC20, LPToken {
    ...
    function test_tokenSwap() public {
        erc20Token1.approveTokens(address(liquidityPool), 500 * ethToWei);
        erc20Token2.approveTokens(address(liquidityPool), 500 * ethToWei);
        liquidityPool.addLiquidity(100 * ethToWei, 200 * ethToWei);

        uint256 amountOut = liquidityPool.swap(20 * ethToWei, address(erc20Token1), address(erc20Token2));
        assertEq(erc20Token2.balanceOf(address(this)), 800 * ethToWei + amountOut, "OK");
    }
    ...
    function testFail_swapTokens() public {
        erc20Token1.approveTokens(address(liquidityPool), 500 * ethToWei);
        erc20Token2.approveTokens(address(liquidityPool), 500 * ethToWei);
        liquidityPool.addLiquidity(100 * ethToWei, 200 * ethToWei);

        uint256 amountOut = liquidityPool.swap(0, address(erc20Token1), address(erc20Token2));
        assertEq(erc20Token2.balanceOf(address(this)), 800 * ethToWei + amountOut, "OK");
    }
